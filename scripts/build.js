...
// Build Linux ARM64
execSync("(go env -w GOOS=linux) && (go env -w GOARCH=arm64) && (go build -o polyn-linux-arm64.bin ./cmd)")
execSync("(cd install) && (go build -o setup-linux-arm64.bin ./cmd)", { stdio: "inherit" });
execSync("(cd uninstall) && (go build -o uninstall-linux-arm64.bin ./cmd)", { stdio: "inherit" });

// Build Linux x64
execSync("(go env -w GOOS=linux) && (go env -w GOARCH=amd64) && (go build -o polyn-linux-x64.bin ./cmd)")
execSync("(cd install) && (go build -o setup-linux-x64.bin ./cmd)", { stdio: "inherit" });
execSync("(cd uninstall) && (go build -o uninstall-linux-x64.bin ./cmd)", { stdio: "inherit" });
