...
func CopyUpgradableFiles(currentBinaryLocation string, home string) error {
	...
	err = exec.Command("cmd", "/c", "copy", currentBinaryLocation+"\\PolyNode\\PolyNode.exe", home+"\\PolyNode\\PolyNode.exe").Run()
	if err != nil {
		return err
	}
	...
	err = exec.Command("cmd", "/c", "xcopy", "/s", "/i", currentBinaryLocation+"\\PolyNode\\gui\\", home+"\\PolyNode\\gui\\").Run()
	if err != nil {
		return err
	}
